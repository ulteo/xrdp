...
void quality_params_prepare_data(struct quality_params* self, struct xrdp_screen* desktop, struct userChannel* u)
{
	...
	{
		self->coef_size = (self->coef_size*0.2 < 0.05) ? 0.05 : self->coef_size*0.2;
	}
	...
	if (bandwidth < 10) self->fps = 5;
	float bw = (u->bandwidth) ? u->bandwidth/self->fps : ((float) (wm->session->bandwidth))/((float) (self->fps));
	...
	else
	{
		...
		while (!fifo_is_empty(desktop->candidate_update_rects))
		{
			...
			if (list_rq->count /*desktop->update_rects->count */> 0) {
				no_inter = true;
				for (i = list_rq->count /*desktop->update_rects->count*/-1; i >= 0 ; i--) {
