	...
	while (merged) {
		...
		for (j = 0 ; j < video->count ; j++) {
			...
			for (i = j ; i < video->count ; i++) {
				...
				if (abs(first->rect->left - second->rect->left) < DELTA_VIDEO &&
					abs(first->rect->right - second->rect->right) < DELTA_VIDEO) {
				...
				if (abs(first->rect->top - second->rect->top) < DELTA_VIDEO &&
					abs(first->rect->bottom - second->rect->bottom) < DELTA_VIDEO) {
		...
		if (self->count > 0) {
			...
			for (i = self->count - 1 ; i >= 0 ; i--) {
				...
				if (!rect_equal(tmp_vr->rect, f_vr->rect)) {
					...
					tmp_vr->nb_update = (tmp_vr->nb_update + 1 > max_fps) ? max_fps : tmp_vr->nb_update + 1 ;
					no_inter = false;
					break;
				}
			}
...
int video_regions_union(struct video_reg* v1, struct video_reg* v2, struct list* out, int maxfps) {
	...
	int nb_update = (v1->nb_update + v2->nb_update > maxfps) ? maxfps : v1->nb_update + v2->nb_update;
