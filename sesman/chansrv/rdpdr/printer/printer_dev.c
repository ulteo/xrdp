...
/*****************************************************************************/
int APP_CC
printer_dev_in_unistr(struct stream* s, char *string, int str_size, int in_len)
{
#ifdef HAVE_ICONV
...
printer_dev_add_printer(http_t* http, char* lp_name, char* device_uri)
{
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_add_printer]: "
  ...
  {
    log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_add_printer]: "
...
printer_dev_del_printer(http_t* http, char* lp_name)
{
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_del_printer]: "
	...
	if ((response = cupsDoRequest(http, request, "/admin/")) == NULL)
	{
	  log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_del_printer]: "
    		" %s", cupsLastErrorString());
		return 1;
	}
	else if (response->request.status.status_code > IPP_OK_CONFLICT)
...
printer_dev_set_ppd(http_t *http, char* lp_name, char* ppd_file)
{
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_set_ppd]: "
  ...
  */
  ...
  if (!strcmp(ppd_file + strlen(ppd_file) - 3, ".gz"))
  {
    ...
    */
    ...
    {
      log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_set_ppd]: "
...
printer_dev_do_operation(http_t * http, int operation, char* lp_name)
{
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_do_operation]: "
  ...
  if (cupsLastError() > IPP_OK_CONFLICT)
  {
    log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_do_operation]: "
...
printer_dev_get_restricted_user_list(http_t* http, char* lp_name, char* user_list)
{
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_get_restricted_user_list]: "
				"get user list for the printer '%s'", lp_name);
  httpAssembleURIf(HTTP_URI_CODING_ALL, uri, sizeof(uri), "ipp", NULL,
                   "localhost", 0, "/printers/%s", lp_name);
  ...
  if (cupsLastError() > IPP_OK_CONFLICT)
  {
    log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_get_restricted_user_list]: "
					"operation failed: %s", ippErrorString(cupsLastError()));
  	ippDelete(response);
  	return 1;
  }
...
printer_dev_set_restrict_user_list(http_t* http, char* lp_name, char* user_list)
{
  ...
  */
  ...
  if (cupsLastError() > IPP_OK_CONFLICT)
  {
    log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_set_restrict_user_list]: "
						"%s", cupsLastErrorString());
    return 1;
  }
  else
								...
								int device_id, char* dos_name)
{
  ...
  if(pnp_name_len != 0)
  {
    printer_dev_in_unistr(s, pnp_name, sizeof(pnp_name), pnp_name_len);
    log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_add]: "
  		  "pnp_name = %s", pnp_name);
  }
  if(driver_name_len != 0)
  {
  	printer_dev_in_unistr(s, driver_name, sizeof(driver_name), driver_name_len);
    log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_add]: "
		  "driver_name = %s", driver_name);
  }
  if(printer_name_len != 0)
  {
  	printer_dev_in_unistr(s, printer_name, sizeof(printer_name), printer_name_len);
    log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_add]: "
  ...
  log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_add]: "
	...
	{
	  log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_add]: "
	...
	{
		log_message(l_config, LOG_LEVEL_ERROR, "rdpdr_printer[printer_dev_add]: "
	...
	{
		g_strncpy(user_list, username, g_strlen(username));
	}
	else
	{
...
printer_dev_get_next_job(char* jobs, int *device_id)
{
  char buf[BUF_LEN];
	...
	if (event->len < 0)
	{
		log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_get_next_job]:"
					"no new jobs in '%s'\n", user_spool_dir);
		return 1;
	}
	log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_get_next_job]:"
...
{
	log_message(l_config, LOG_LEVEL_DEBUG, "rdpdr_printer[printer_dev_delete_job]:"
				"delete job '%s'", jobs);
	if(g_file_delete(jobs) == 0)
	{
		log_message(l_config, LOG_LEVEL_WARNING, "rdpdr_printer[printer_dev_delete_job]:"
...
printer_dev_init_printer_socket( char* printer_name)
{
	...
	{
		log_message(l_config, LOG_LEVEL_ERROR, "rdpdr_printer[printer_dev_init_printer_socket]:"
